...
class Fredformer_backbone(nn.Module):
    def __init__(self, configs,  global_mask, use_nys:int, output:int, cf_dim:int,cf_depth :int,cf_heads:int,cf_mlp:int,cf_head_dim:int,cf_drop:float,c_in:int, context_window:int, target_window:int, patch_len:int, stride:int,  d_model:int,
                head_dropout = 0,individual = False, revin = True, affine = True, subtract_last = False, **kwargs):

        super().__init__()
        self.task_name = 'long_term_forecast'
        self.use_nys = use_nys
        self.global_mask = global_mask
        self.filter = FrequencyDomainFilter(configs, self.global_mask)
        # RevIn
        self.revin = revin
